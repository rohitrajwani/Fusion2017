...
@section('header')
	...
	function answer_question(ques_id)
	{
		if(xmlHttp.readyState==0 || xmlHttp.readyState==4)
		{
			xmlHttp.open('GET','/online_quizzing/answer_question/'+ques_id+'/'+{{ $student_id }},true);
...
@section('content')
	...
	<div class="main-container row">
		<!-- <div class="col l4"> style="width:700px;float:center"   -->
			...
			<?php $i=0;
					$/online_quizzing/quiz_questions=$quiz->questions;
					$/online_quizzing/quiz_questions=$/online_quizzing/quiz_questions->shuffle();
					$len=count($/online_quizzing/quiz_questions);
				?>
			...
			<div id="timer" class="center-align" style='font-size: 50px;'>Timer</div>
				...
				@foreach($/online_quizzing/quiz_questions as $question)
					<li>
					<div class="col l3">
						<span class="waves-effect btn qno" onclick="answer_question( {{ $question->id }} ) "><?php 
					echo $i+1; $i++; ?></span>
					</div></li>
				@endforeach
				</ul>
			...
			<div class="center-align">
					...
					<div id="modal1" class="modal">
						...
						<div class="modal-footer">
							...
							<a href="/online_quizzing/submit_quiz/{{ $student_id }}/{{ $quiz->id }}" class="modal-action modal-close waves-effect btn-flat">Yes</a>
						</div>
		...
		function timer()
	{
		...
		if(hours<0)
		{
			window.location.href="<?php echo '/online_quizzing/submit_quiz/'.$student_id.'/'.$quiz->id; ?>";
		}
		else
		{
			document.getElementById('timer').innerHTML=hours+" hrs "+mins+" mins "+secs+" secs ";
