...
<!--
  ...
  Following section allows VH incharge to look in to the Request history for Vh rooms
...
<div class="row">
    ...
    <div class="container">      
      <div class="col s6">
        <form class="s12" method="POST" action="/{{$booking->id}}/approve">
          <input type="hidden" name="_token" value="{{ csrf_token() }}" />        
              <button type="submit"  class="waves-effect btn">Approve</button>        
        </form>
      </div>

      <div class="col s6">
        <form class="s12" method="POST" action="/{{$booking->id}}/approveNot">
          <input type="hidden" name="_token" value="{{ csrf_token() }}" />        
              <button type="submit" class="waves-effect btn">Decline</button>        
        </form>
      </div>
    </div>
