...
@section('main_content')
	<?php
        ...
        $table = \DB::table('employee_leave')->where([['leave_granting_officer','=',\Auth::user()->username],['status','=',0]])->get();
        $counter = 0;
    ...
    <nav class="mynav">
        <div class="nav-wrapper">
            <ul>
                ...
                <li><a href="/ELMS/historyStaffLGO">Leave History</a></li>
                <li><a href="/ELMS/statusStaffLGO">Check Status</a></li>
                <li class="active"><a href="/ELMS/requestsStaffLGO">Pending Leave Requests<span class="badge">{{$counter}}</span></a></li>
    ...
    <table class="bordered highlight">
	    ...
	    ?>
	     @foreach($leavedet as $leave)
	      	...
	      	?>
	      <tr>
		      <form action="/ELMS/grantingStaff/{{$leave->app_id}}" method="post">
		        <!-- <td>{{$counter}}</td> -->
		        <td>{{$leave->app_id}}</td>
		        <td>{{$leave->user_id}}</td> 
		        @if($f == 1)
