...
class stock extends Controller{
	...
	public function stockhome(){
    	...
    	return view('stock.home',['un'=>$un,'ut'=>$ut]);
    }
    public function newpage(){
    	$un = Auth::user()->username;
    	 $ut = Auth::user()->user_type;
    	 return view('stock.new',['un'=>$un,'ut'=>$ut]);
...
$un = Auth::user()->username;
		...
		return view('stock.view',['stock'=>$stock,'un'=>$un,'ut'=>$ut]);
	}
	else {
		return Redirect::to('/stock_management/');
	}
	}
...
$un = Auth::user()->username;
    	 $ut = Auth::user()->user_type;
	return view('stock.existing',['item'=>$item,'error'=>'0','un'=>$un,'ut'=>$ut]);
}
...
$un = Auth::user()->username;
	...
	$query = DB::table('inventory')->where('item_id','=',$request['itemid'])->update(['item_in_hand'=> $quantity + $request['quantity']]);
		...
		if ($request['quantity'] <= $quantity){
			...
			$item = DB::table('inventory')->get();
				return view('stock.existing',['item'=>$item,'error'=>'1','un'=>$un,'ut'=>$ut]);
		}
	}
	return Redirect::to('/stock_management/');
}
...
$un = Auth::user()->username;
		...
		if($ut == "student"){
		return view('stock.requests',['item'=>$item,'prof'=>$prof,'un'=>$un,'ut'=>$ut]);
		}
		else {
			return Redirect::to('/stock_management/')->with('alert','You Cannot Request Items');
		}
	}
	...
	public function chrhome(){
		...
		return view('/stock.checkreq',['req'=>$req,'item'=>$itemname,'un'=>$un,'ut'=>$ut]);
	}
	...
	public function status($req){
		...
		return view('stock.statcheck',['pending'=>$pending,'itid'=>$itid,'un'=>$un,'ut'=>$ut]);
	}
