...
class FusionDB extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::create('Student', function(Blueprint $table) {
            ...
            $table->string('health_insurance_id', 100);
        ...
        Schema::create('Mess_Committee', function(Blueprint $table) {
            ...
            $table->string('student_id', 100);
            $table->string('mess_id', 20);
            $table->primary(['student_id','mess_id']);
            $table->foreign('student_id')->references('student_id')->on('Student');

            $table->timestamps();
        });

        Schema::create('Mess_Complaints', function(Blueprint $table) {
        ...
        Schema::create('Ta', function(Blueprint $table) {
            ...
            $table->foreign('student_id')->references('student_id')->on('Student');
            $table->primary(['student_id','course_id','batch']);
            $table->timestamps();
        });
        Schema::create('Ta_Attendance_Sheet', function(Blueprint $table) {
            
            $table->string('course_id', 100);
            $table->date('date');
            $table->time('time');
            $table->integer('batch');
            $table->string('att_id', 100);
            $table->foreign('course_id')->references('course_id')->on('Course');
            $table->primary('att_id');
            $table->timestamps();
        });
        ...
        Schema::create('Ta_Attendance', function(Blueprint $table) {
            ...
            $table->primary(['att_id','student_id']);
            $table->foreign('student_id')->references('student_id')->on('Student');
            $table->foreign('att_id')->references('att_id')->on('Ta_Attendance_Sheet');
            $table->timestamps();
        });
